...
- ✅ **Визуальные метки** — ⭐️ для оплативших за месяц и 🏐 для донатеров на мячи
...
6. **Хранение состояния:** Активные опросы и флаги бота сохраняются в SQLite (`volleybot.db`). Путь к базе можно поменять через переменную окружения `VOLLEYBOT_DB_PATH`. Файл базы добавлен в `.gitignore`.
