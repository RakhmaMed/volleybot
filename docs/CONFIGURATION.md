...
На данный момент управление опросами осуществляется напрямую через таблицу `poll_templates` в базе данных `volleybot.db` с помощью любого SQLite клиента. Связанные таблицы `poll_subscriptions`, `hall_payments` и `transactions` используют `poll_template_id`, поэтому переименование `name` больше не требует ручного обновления внешних ссылок. В будущих версиях будут добавлены административные команды в интерфейс бота.
